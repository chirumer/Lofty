...
let cachedDocumentAssets: ReturnType<typeof readLoftyDocumentAssets> | null = null;
...
function parseStyleAttribute(styleText: string) {
  if (!styleText) {
    return undefined;
  }
  ...
  return decoded.split(";").reduce<Record<string, string>>((acc, declaration) => {
...
function getTagAttributes(tagText: string) {
  return [...tagText.matchAll(/([^\s=/>]+)(?:="([^"]*)")?/g)].reduce<Record<string, string>>((acc, match) => {
...
function readLoftyDocumentAssets() {
...
export function getLoftyDocumentAssets() {
  ...
  cachedDocumentAssets = readLoftyDocumentAssets();
