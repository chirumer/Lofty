  ...
  const workflowTasks = useMemo(() => {
    ...
    const taskItems = tasks
      .map((task, index) => {
        ...
        if (taskType === "Call") priorityScore += 14;
        if (hasHighIntent) priorityScore += 16;
        if (hasSellerSignal) priorityScore += 10;
        if (isPrepHeavy) priorityScore += 8;
        if (task.timeLabel === "Anytime") priorityScore -= 4;
        if (explicitTime !== null) priorityScore += 10;
        ...
        return {
          ...
          metadata: [person.roles.join(" · "), person.segments?.[0] ?? person.leadType, person.lastReply],
