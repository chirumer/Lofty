...
const triggerDefinitions: SmartPlanTriggerDefinition[] = [
    ...
    criteriaOptions: [
      {
        ...
        valueOptions: ["Website", "Email Parsing", "Facebook", "Google", "Zillow", "Realtor.com", "Open House"]
      },
      {
        field: "Tag",
      ...
      },
      {
        field: "Pipeline",
        logicOptions: ["is", "is not"],
        valueOptions: ["New", "Attempting Contact", "Nurture", "Active Client"]
      }
    ]
  },
  {
    id: "lead-assignment-changed",
    category: "Lead Change",
    label: "Lead Assignment Changed",
    description: "Use this when a lead gets assigned or reassigned.",
    criteriaOptions: [
      {
        field: "With Another Agent",
        logicOptions: ["is", "is not"],
        valueOptions: ["Primary Agent", "Lender", "ISA", "Showing Agent"]
      }
    ]
  },
  {
  ...
  },
  {
    id: "segment-changed",
    category: "Lead Change",
    label: "Segment Changed",
    description: "Trigger when a lead enters or leaves a segment.",
    criteriaOptions: [
      {
        field: "Segment",
        logicOptions: ["is", "is not"],
        valueOptions: ["Warm buyers", "Likely sellers", "Past clients", "Long-term nurture"]
      }
    ]
  },
  {
    id: "contact-permission-changed",
    category: "Lead Change",
    label: "Contact Permission Changed",
    description: "Start a plan when contact permission becomes available.",
    criteriaOptions: [
      {
        field: "Permission",
        logicOptions: ["is"],
        valueOptions: ["Allowed", "Restricted", "Text only", "Email only"]
      }
    ]
  },
  {
    id: "lead-meets-criteria",
    category: "Lead Change",
    label: "Lead Meets Specified Criteria",
    description: "Set your own mix of lead rules and let Lofty apply the plan.",
    criteriaOptions: [
      {
        field: "Property Type",
        logicOptions: ["is", "is not"],
        valueOptions: ["Single Family", "Condo", "Townhome", "Land"]
      },
      {
        field: "Time Frame",
        logicOptions: ["is"],
        valueOptions: ["0-3 months", "3-6 months", "6-12 months", "12+ months"]
      },
      {
        field: "Source",
        logicOptions: ["is", "is not"],
        valueOptions: ["Website", "Open House", "Google", "Facebook"]
      }
    ]
  },
  {
    ...
    label: "Last Touch Type",
    description: "Branch from the latest lead interaction.",
    criteriaOptions: [
      {
        field: "Touch Type",
        logicOptions: ["is"],
        valueOptions: ["Call", "Email", "Text", "Website visit"]
      }
    ]
  },
  {
...
const actionDefinitions: SmartPlanActionDefinition[] = [
  {
    ...
    fields: [
      ...
      { id: "subject", label: "Email Subject", type: "text", defaultValue: "Your inquiry about 123 Main St!" },
      {
        ...
        type: "textarea",
        defaultValue:
          "Hi Sarah,\n\nThanks for reaching out about 123 Main St! I just received your request. To answer your question: Yes, it is currently still available. I have a few openings for tours tomorrow. Would morning or afternoon work better for you?"
      },
  ...
  },
  {
    ...
    fields: [
      { id: "sender", label: "Send From", type: "select", options: ["Assigned Agent", "Team number"], defaultValue: "Assigned Agent" },
      {
        ...
        label: "Message",
        type: "textarea",
        defaultValue: "Hi {{lead_name}}, thanks for your inquiry. I just sent you an email with the details and I can line up a tour whenever you're ready."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      { id: "prompt", label: "Instructions", type: "textarea", defaultValue: "Write a short follow-up email that confirms availability, offers a tour, and asks the lead's buying timeline." }
    ]
  },
  {
    id: "start-smart-plan",
...
const templateCards: SmartPlanTemplate[] = [
  ...
  {
    id: "buyer-just-looking",
    name: "Buyer-Just Looking (AI)",
    leadTypeSummary: "Include one of: Buyer",
    duration: "238 days duration",
    badges: [<Phone key="call" size={16} />, <Mail key="mail" size={16} />, <Bot key="bot" size={16} />],
    buildDraft: (scope) =>
      buildPlanDraft(scope, {
        name: "Buyer-Just Looking (AI)",
        targetLeadTypeMode: "Include One Of",
...
function SmartPlanField({
  ...
  if (field.type === "textarea") {
    return (
      <textarea value={String(value ?? "")} rows={7} onChange={(event) => onChange(event.target.value)} />
    );
  }
...
export default function SmartPlansWorkspace({ role }: { role: RoleDefinition }) {
  ...
  function startFromTemplateCard(template: SmartPlanTemplate) {
    openCreateFlow("template", template);
  }
  ...
  function addTriggerCriteriaRow(triggerId: string) {
    ...
    const nextDraft: SmartPlanTrigger = {
      definitionId: definition.id,
      label: definition.label,
      criteria: [
        {
    ...
    setDrawer({ type: "trigger-detail", draft: nextDraft });
  }
    ...
    return (
      <section className="smartplans-builder-page">
        <header className="smartplans-builder-header">
          <button className="smartplans-back-button" onClick={() => setSurface("index")}>
            <ArrowLeft size={20} />
          </button>
        ...
        <div className="smartplans-builder-layout">
          <aside className="smartplans-settings-card">
            ...
            <div className="smartplans-setting-block">
              <button className="smartplans-setting-toggle">
            ...
            <div className="smartplans-setting-block smartplans-setting-block--expanded">
              ...
              <div className="smartplans-setting-content">
                <select
                  ...
                  onChange={(event) =>
                    setBuilder({ ...builder, targetLeadTypeMode: event.target.value as LeadTypeMode })
                  }
                >
                ...
                <div className="smartplans-setting-actions">
                  <button className="smartplans-subtle-button">Cancel</button>
                  <button className="smartplans-primary-inline">Save</button>
                </div>
              </div>
            </div>
              ...
              {!builder.trigger ? (
                <div className="smartplans-step-chooser smartplans-step-chooser--initial">
                  <div className="smartplans-step-chooser-header">
                    ...
                    <button className="smartplans-ghost-icon" onClick={() => setStepMenuTarget(null)}>
                      <X size={18} />
                    </button>
                  </div>
                  <div className="smartplans-step-chooser-grid">
                    <button className="smartplans-step-choice smartplans-step-choice--trigger" onClick={() => setDrawer({ type: "trigger-list" })}>
                      <Target size={18} />
                      Trigger
                    </button>
                    <button className="smartplans-step-choice smartplans-step-choice--disabled" disabled>
                      <Clock3 size={18} />
                      Condition
                    </button>
                    <button className="smartplans-step-choice smartplans-step-choice--disabled" disabled>
                ...
                <div className="smartplans-flow-column">
                  ...
                  <button className="smartplans-plus-node" onClick={() => setStepMenuTarget("after-trigger")}>
                    <Plus size={20} />
                  </button>
                  ...
                  {stepMenuTarget === "after-trigger" ? (
                    <div className="smartplans-step-chooser">
                      ...
                      <div className="smartplans-step-chooser-grid">
                        <button className="smartplans-step-choice" onClick={() => setDrawer({ type: "trigger-list" })}>
                          <Target size={18} />
                          Trigger
                        </button>
                        <button className="smartplans-step-choice smartplans-step-choice--condition" onClick={() => setDrawer({ type: "condition-list" })}>
                          <Clock3 size={18} />
                          Condition
                        </button>
                        <button className="smartplans-step-choice smartplans-step-choice--action" onClick={() => setDrawer({ type: "action-list" })}>
                          <Workflow size={18} />
                          Action
                        </button>
                      </div>
                  ...
                  {builder.steps.map((step, index) => (
                    <div key={step.id} className="smartplans-step-stack">
                      ...
                      <div className="smartplans-flow-line" />
                      <button className="smartplans-plus-node" onClick={() => setStepMenuTarget(index)}>
                        <Plus size={20} />
                      </button>
          ...
          <aside className="smartplans-drawer">
            ...
            {drawer.type === "trigger-list" ? (
              <>
                <div className="smartplans-drawer-header">
                  <h3>Add a Trigger</h3>
                  <button className="smartplans-ghost-icon" onClick={() => setDrawer({ type: "none" })}>
                    <X size={18} />
                  </button>
                </div>
                  ...
                  <SmartPlansSection key={category} title={category} actions={<ChevronDown size={18} />}>
                    <div className="smartplans-option-list">
                      {triggerDefinitions
                        ...
                        .map((trigger) => (
                          <button key={trigger.id} className="smartplans-option-card" onClick={() => addTriggerCriteriaRow(trigger.id)}>
              ...
              <>
                <div className="smartplans-drawer-header">
                  <div className="smartplans-drawer-back-title">
                    <button className="smartplans-ghost-icon" onClick={() => setDrawer({ type: "trigger-list" })}>
                      <ArrowLeft size={18} />
                    </button>
                ...
                <div className="smartplans-drawer-footer">
                  <button className="smartplans-subtle-button" onClick={() => setDrawer({ type: "none" })}>
                    Cancel
                  </button>
                  <button className="smartplans-primary-inline" onClick={() => saveTrigger(drawer.draft)}>
                    Save
                  </button>
                </div>
              </>
                  ...
                  <SmartPlansSection key={group} title={group} actions={<ChevronDown size={18} />}>
                    <div className="smartplans-option-list">
                      {actionDefinitions
                        ...
                        .map((action) => (
                          <button key={action.id} className="smartplans-option-card" onClick={() => openActionConfig(action)}>
                            <div className="smartplans-option-card-main">
                ...
                <SmartPlansSection title="Condition" actions={<ChevronDown size={18} />}>
                  <div className="smartplans-option-list">
                    {conditionDefinitions.map((condition) => (
                      <button key={condition.id} className="smartplans-option-card" onClick={() => openConditionConfig(condition)}>
                        <div className="smartplans-option-card-main">
            ...
            {drawer.type === "action-detail" ? (
              <>
                <div className="smartplans-drawer-header">
                  <div className="smartplans-drawer-back-title">
                    <button className="smartplans-ghost-icon" onClick={() => setDrawer({ type: "action-list" })}>
                      <ArrowLeft size={18} />
                    </button>
                          ...
                          {availableDrawerVariables.map((variable) => (
                            <button
                              ...
                              onClick={() => {
                                const targetFieldId =
                                  drawer.definition.id === "auto-email"
                                    ? drawer.insertTargetFieldId ?? "body"
                                    : "message";
                ...
                <div className="smartplans-drawer-footer">
                  ...
                  <button className="smartplans-primary-inline" onClick={() => saveAction(drawer.definition, drawer.draft)}>
                    Save
                  </button>
                </div>
              </>
            ...
            {drawer.type === "condition-detail" ? (
              <>
                <div className="smartplans-drawer-header">
                  <div className="smartplans-drawer-back-title">
                    <button className="smartplans-ghost-icon" onClick={() => setDrawer({ type: "condition-list" })}>
                      <ArrowLeft size={18} />
                    </button>
                    <h3>{drawer.definition.label}</h3>
                  </div>
                ...
                <div className="smartplans-drawer-footer">
                  ...
                  <button className="smartplans-primary-inline" onClick={() => saveCondition(drawer.definition, drawer.draft)}>
                    Save
                  </button>
                </div>
              </>
    ...
    <section className="smartplans-page">
      <header className="smartplans-index-header">
        ...
        <div className="smartplans-index-switcher">
          <button className={indexTab === "plans" ? "smartplans-index-switcher-button is-active" : "smartplans-index-switcher-button"} onClick={() => setIndexTab("plans")}>
            Plans
          </button>
          <button className={indexTab === "library" ? "smartplans-index-switcher-button is-active" : "smartplans-index-switcher-button"} onClick={() => setIndexTab("library")}>
            Library
          </button>
        </div>
        ...
        <div className="smartplans-create-box">
          {indexTab === "plans" ? (
            <div className="smartplans-create-menu">
              <button className="smartplans-create-button" onClick={() => setCreateMenuOpen((open) => !open)}>
                <Plus size={18} />
                Create Smart Plan
                <ChevronDown size={18} />
              </button>
              {createMenuOpen ? (
                <div className="smartplans-create-dropdown">
                  <button onClick={() => openCreateFlow("scratch")}>From Scratch</button>
                  <button
                    onClick={() => {
                      setIndexTab("library");
                      setCreateMenuOpen(false);
            ...
            <button className="smartplans-create-button" onClick={() => openCreateFlow("scratch")}>
              <Plus size={18} />
              Create From Scratch
            </button>
          )}
      ...
      {indexTab === "plans" ? (
        <div className="smartplans-index-layout">
          <aside className="smartplans-sidebar">
            ...
            <button className="smartplans-sidebar-item smartplans-sidebar-item--active">
              <LayoutGrid size={18} />
              All Smart Plans
            </button>
            ...
            <button className="smartplans-folder-add">
              <FolderPlus size={18} />
              Add Folder
            </button>
          </aside>
          ...
          <div className="smartplans-main-panel">
            <div className="smartplans-toolbar">
              ...
              <button className="smartplans-filter-button">
                Auto Apply: All
                <ChevronDown size={16} />
              </button>
              <button className="smartplans-filter-button">
                ...
                {filteredPlans.map((plan) => (
                  <div key={plan.id} className="smartplans-table-row">
                    ...
                    <div className="smartplans-table-actions">
                      <button className="smartplans-ghost-icon" onClick={() => setIndexTab("library")}>
                        <Eye size={17} />
                      </button>
                      <button className="smartplans-ghost-icon" onClick={() => openCreateFlow("scratch")}>
                        <Settings2 size={17} />
                      </button>
                    </div>
                  </div>
            ...
            {filteredTemplates.map((template) => (
              <article key={template.id} className="smartplans-template-card">
                ...
                <div className="smartplans-template-actions">
                  <button className="smartplans-template-use" onClick={() => startFromTemplateCard(template)}>
                    Use Template
                  </button>
                  <button className="smartplans-template-preview" onClick={() => openCreateFlow("template", template)}>
                    <Eye size={16} />
                  </button>
                </div>
              </article>
        ...
        <div className="smartplans-modal-backdrop" onClick={() => setScopeModalOpen(false)}>
          <div className="smartplans-modal" onClick={(event) => event.stopPropagation()}>
            <div className="smartplans-modal-header">
              <h2>CREATE A PLAN</h2>
              <button className="smartplans-ghost-icon" onClick={() => setScopeModalOpen(false)}>
                <X size={18} />
              </button>
            </div>
            ...
            <div className="smartplans-modal-footer">
              <button className="smartplans-subtle-button" onClick={() => setScopeModalOpen(false)}>
                Cancel
              </button>
              <button className="smartplans-save-button smartplans-save-button--inline" onClick={startBuilderFromScope}>
                Create Plan
